...
def seed_everything(seed=42):
    ...
    # 针对 CuDNN 卷积底层的终极封印 (这会稍微牺牲一点点极速，但换取了100%的稳定性)
...
def evaluate(model, val_loader, device, criterion, mode='DIVAN'):
    ...
    #     # 必须同时包含正负样本
    #     if len(np.unique(labels)) > 1:
    #         group_aucs.append(roc_auc_score(labels, preds))
    ...
    #         ndcg_5s.append(calculate_ndcg_at_k(labels, preds_with_noise, k=5))
    #         ndcg_10s.append(calculate_ndcg_at_k(labels, preds_with_noise, k=10))
    # print(f"正在调用全核 CPU 并发计算指标，组数: {len(grouped_preds)}...")
...
def main(model="DIVAN"):
    ...
    # ==========================================
    print("\n正在打包 DataLoader...")
    # train_dataset = EbnerdDataset(train_behvior_path, news_feat_dict, train_history_dict)
    train_dataset = EbnerdDataset(train_behvior_path)
    print("train Dataset 初始化完成")
    # val_dataset = EbnerdDataset(val_behvior_path, news_feat_dict, val_history_dict)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch_dict in enumerate(train_pbar):
            ...
            # 3. 计算误差 (Loss)
            # loss = criterion(y_pred, batch_dict['label'])
            ...
            if batch_idx > 0 and batch_idx % EVAL_STEP_FREQ == 0:
                # tqdm.write(f"Epoch [{epoch+1}/{num_epochs}] | Step [{batch_idx}/{len(train_loader)}] | Loss: {loss.item():.4f}")
                ...
                # 极其关键：验证跑完后，模型变成了 eval 模式，必须强行切回 train 模式！
                model.train()
